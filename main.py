...
def parse_arguments():
    ...
    required_args.add_argument('-b', '--base_scores', type=base_scores_type,
                               ...
                               required=True)
    required_args.add_argument('-o', '--output', type=str, help="输出路径", required=True)
    ...
    optional_args.add_argument('-v', '--verbose', type=lambda x: (str(x).lower() == 'true'), default=False,
                               help="是否输出日志 (True 或 False)，默认为 False")
